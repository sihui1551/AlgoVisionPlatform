(function () {
  ...
  window.registerPrototypePage({
    ...
    productDoc: {
      title: "事件列表需求说明",
      summary: "面向研发、设计、测试说明事件中心列表页的原型范围。",
      goal: "供运营人员统一查看视频流分析与图片分析产生的事件，重点支持按类型切换、筛选排序和事件详情查看。",
      modules: [
        "顶部标签在视频流分析与图片分析事件之间切换，复用统一事件管理框架。",
        "筛选区支持收藏过滤、排序和视图切换，用于快速定位目标事件。",
        "事件列表用于展示事件类型、点位、关联任务、时间、推送状态等核心信息。",
        "详情弹窗用于查看事件图片、缩略图、基础信息和地图位置。"
      ],
      rules: [
        "列表页以事件浏览和判断为主，不在当前页直接处理复杂处置流程。",
        "视频流与图片事件共用详情结构，但保留各自输入方式与素材差异。",
        "排序、收藏过滤和详情浏览需要围绕当前活动标签独立生效。"
      ],
      interactions: [
        "切换事件标签后，列表数据和统计数量需要同步更新。",
        "调整排序、收藏过滤或视图方式后，列表结果需要正确刷新。",
        "点击事件详情可打开详情弹窗，并支持主图、缩略图和地图信息切换。",
        "详情弹窗中的预览、回放、下载和全屏按钮需要给出反馈。"
      ]
    },
    countTextPrefix: "共",
    countTextUnit: "条",
