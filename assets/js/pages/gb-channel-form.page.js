(function () {
  ...
  window.registerPrototypePage({
    key: "gb-channel-edit",
    kind: "dashboard",
    styleSource: "../assets/css/pages/gb-channel-form.css",
    heading: "编辑通道",
    subtitle: "维护通道基础信息、注册属性和扩展能力配置。",
    breadcrumbTrail: ["接入管理", "接入源管理", "通道列表"],
    productDoc: {
      title: "通道编辑需求说明",
      summary: "面向研发、设计、测试说明国标通道编辑页的原型范围。",
      goal: "供平台管理员维护国标通道的基础信息、注册属性和扩展能力配置。",
      modules: [
        "表单主体按字段分组展示通道名称、编码、位置、设备状态和扩展能力等配置项。",
        "输入组件同时覆盖文本、下拉、带操作按钮输入和勾选项，用于表达复杂表单能力。",
        "底部操作区提供确认与取消入口，承接保存和返回通道列表流程。"
      ],
      rules: [
        "当前页面以表单维护为主，不承载通道实时预览与运行监控能力。",
        "必填字段和默认值需要在页面初始化时正确回填。",
        "保存后返回列表页，重置动作需要恢复当前表单默认值。"
      ],
      interactions: [
        "点击确认后，需要给出保存成功反馈并返回通道列表。",
        "点击取消后，需要直接返回通道列表而不修改当前结果。",
        "点击重置后，表单内容需要恢复到默认值。",
        "生成或选择类输入按钮需要给出对应占位反馈。"
      ]
    },
    gbChannelFormPage: {
      title: "编辑通道",
