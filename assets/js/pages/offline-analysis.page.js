window.registerPrototypePage({
  ...
  productDoc: {
    title: "离线视图分析需求说明",
    summary: "面向研发、设计、测试说明离线分析任务管理页的原型范围。",
    goal: "供运营人员统一查看并管理离线批量素材分析任务，重点支持任务筛选、执行状态判断和任务入口跳转。",
    modules: [
      "顶部工具栏提供新建任务、启动和删除操作，承接离线任务管理主动作。",
      "筛选区支持按任务 ID、任务名称、执行状态和素材类型定位目标任务。",
      "任务列表集中展示输入类型、素材类型、文件数量、分析算法、执行状态、结果状态和提交时间。",
      "行内操作用于查看详情、启动或删除任务。"
    ],
    rules: [
      "列表页以任务管理和执行状态判断为主，不承载复杂参数编辑。",
      "执行状态与结果状态需要分开表达，便于区分处理进度与输出结果。",
      "新建任务通过独立页面完成，启动和删除仅作用于当前选中或当前行任务。"
    ],
    interactions: [
      "点击新建任务可进入离线任务创建页。",
      "启动和删除操作仅对当前勾选项或当前行生效。",
      "筛选条件变化后，列表结果需要正确刷新。",
      "不同执行状态下，行内操作需要保持与状态匹配。"
    ]
  },
  countTextPrefix: "共",
  countTextUnit: "条记录",
