(function () {
  ...
  window.registerPrototypePage({
    ...
    productDoc: {
      title: "算法管理需求说明",
      summary: "面向研发、设计、测试说明算法资产管理页的原型范围。",
      goal: "供算法运营和平台管理员统一维护算法资产，重点支持算法检索、状态管理和基础台账维护。",
      modules: [
        "顶部工具栏提供新增、禁用、删除操作，承接算法维护主动作。",
        "筛选区支持按分析类型、分析源类型和关键字定位算法。",
        "算法列表集中展示算法 ID、名称、标识、适配源类型、版本和创建时间。",
        "详情弹窗用于核对负责人、运行环境、准确率和场景说明。"
      ],
      rules: [
        "列表页以检索和维护为主，不承载模型包管理等深度运维能力。",
        "算法 ID 与算法标识在页面范围内需要保持唯一。",
        "禁用算法后保留记录，仅阻止其继续被新任务引用。"
      ],
      interactions: [
        "关键字搜索可匹配算法 ID、算法名称与算法标识。",
        "点击详情可打开算法详情弹窗。",
        "新增算法保存后，列表需要即时刷新。",
        "删除操作仅对当前勾选项生效。"
      ]
