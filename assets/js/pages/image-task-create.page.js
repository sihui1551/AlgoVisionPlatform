window.registerPrototypePage({
  ...
  "productDoc": {
    "title": "图片分析任务需求说明",
    "summary": "该页面用于创建图片分析任务，重点包括算法选择、有效期、优先级、告警推送、输入方式和点位配置。",
    "goal": "通过独立任务创建页完成图片分析任务的基础配置与算法点位绑定。",
    "modules": [
      "分析算法：点击【选择算法】弹出算法列表，只展示支持图片分析的算法，可多选后保存。",
      "分析有效期：用于控制任务是否长期运行或在指定时间范围内生效。",
      "告警推送：点击【选择地址】打开地址列表，支持多选地址并可新增推送地址。",
      "计算资源配置：支持资源规格选择，并保持与现有任务页风格一致。",
      "输入方式：支持 SDK 抓图、API 和消息队列三种接入方式。",
      "分析点位：根据算法配置展示点位列表，支持预览、编辑和删除。"
    ],
    "rules": [
    ...
    "interactions": [
      ...
      "点击选择地址后，在地址列表中勾选需要推送的平台，也可以继续新建地址。",
      "时间范围在选择长期后自动禁用日期输入。",
      "分析点位支持从设备点位池中添加，列表内支持预览、编辑和删除。"
    ]
  },
  "taskBuilder": {
