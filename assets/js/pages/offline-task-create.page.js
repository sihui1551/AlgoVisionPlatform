(function () {
  ...
  window.registerPrototypePage({
    ...
    productDoc: {
      title: "离线任务创建需求说明",
      summary: "面向研发、设计、测试说明离线分析任务创建页的原型范围。",
      goal: "通过独立创建页完成离线素材分析任务的基础信息录入、算法组合选择、素材绑定和资源配置。",
      modules: [
        "基础表单负责录入任务名称、素材类型、优先级和告警接收方等公共信息。",
        "算法选择区支持从候选算法中添加或移除分析算法组合。",
        "资源规格区用于选择离线任务执行所需的算力规格。",
        "素材配置区用于从本地视频页同步素材，并管理已选素材记录。"
      ],
      rules: [
        "提交前需要至少选择一种分析算法并绑定分析素材。",
        "素材类型切换后，候选算法和素材表达需要与当前类型保持一致。",
        "离线任务创建页负责配置与提交流程，不承载结果查看能力。"
      ],
      interactions: [
        "点击选择算法或接收方时，需要打开对应弹窗并在保存后回写结果。",
        "点击添加素材时，需要进入本地视频选择流程或同步已上传素材。",
        "未选择必要算法或素材直接提交时，需要阻止提交并给出提示。",
