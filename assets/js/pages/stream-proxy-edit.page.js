(function () {
  ...
  function registerPage(meta) {
    window.registerPrototypePage({
      key: meta.key,
      kind: "dashboard",
      styleSource: meta.mode === "create" ? "../assets/css/pages/stream-proxy-create.css" : "../assets/css/pages/stream-proxy-edit.css",
      heading: meta.heading,
      subtitle: meta.subtitle,
      breadcrumbTrail: ["接入管理", "接入源管理"],
      productDoc: {
        title: meta.heading + "需求说明",
        summary: "面向研发、设计、测试说明拉流代理配置页的原型范围。",
        goal: "供平台管理员完成拉流代理的新增或编辑，重点维护代理基础参数和关联国标通道配置。",
        modules: [
          "拉流代理信息页签用于维护代理名称、拉流节点、拉流方式和无人观看策略等基础参数。",
          "编辑模式下补充国标通道配置页签，用于维护关联通道的基础信息、注册属性和扩展能力。",
          "底部保存入口用于提交配置并返回接入源管理页。"
        ],
        rules: [
          "新增模式仅维护拉流代理基础信息，编辑模式在此基础上增加国标通道配置。",
          "页签切换需要保留当前草稿内容，不因切换而丢失。",
          "保存后返回接入源管理页，当前页面不承载实时播放与运行监控能力。"
        ],
        interactions: [
          "新增和编辑模式进入页面时，需要展示对应标题、按钮文案和页签结构。",
          "点击页签后，需要正确切换内容区域并保留当前草稿。",
          "点击保存后，需要给出成功反馈并返回接入源管理页。"
        ]
      },
      streamProxyEditPage: {
        mode: meta.mode,
