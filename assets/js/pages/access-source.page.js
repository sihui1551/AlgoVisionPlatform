(function () {
  ...
  window.registerPrototypePage({
    key: "access-source",
    kind: "table",
    styleSource: "../assets/css/pages/access-source.css",
    heading: "接入源管理",
    subtitle: "统一管理国标设备与拉流设备，按类型继承公共属性并扩展个性配置。",
    breadcrumbTrail: ["接入管理"],
    productDoc: {
      title: "接入源管理需求说明",
      summary: "面向研发、设计、测试说明接入源统一管理页的原型范围。",
      goal: "供平台管理员统一查看并维护国标设备与拉流设备，重点支持按类型筛选、状态查看和新增维护入口。",
      modules: [
        "顶部标签按接入源类型区分国标设备与拉流设备，并复用统一的筛选与列表框架。",
        "筛选区支持按关键字、运行状态和启用状态定位目标接入源。",
        "列表按接入源类型切换字段展示，承接详情、启停、编辑、通道查看和播放等操作入口。",
        "详情弹窗、创建弹窗和播放器弹窗分别用于查看信息、补录参数和预览拉流效果。"
      ],
      rules: [
        "列表页负责统一管理入口，不在当前页直接展开完整设备或代理表单。",
        "国标设备与拉流设备共用公共状态筛选，但保留各自独立字段与个性配置。",
        "新增或编辑动作通过弹窗或独立页面承接，播放器预览与基础维护逻辑分开表达。"
      ],
      interactions: [
        "切换接入源类型后，列表字段和筛选结果需要同步更新。",
        "点击新增接入源可打开创建流程，并按选择的类型进入对应配置。",
        "点击详情、编辑、通道列表或播放入口时，需要进入对应弹窗或页面。",
        "筛选条件变化后，列表结果需要正确刷新。"
      ]
    },
    countTextPrefix: "共 ",
    ...
    renderTablePanel: function (ctx) {
      ...
      return '<section class="panel access-source-panel">' +
        ...
        '</div><div class="toolbar-group access-source-actions"><button id="access-source-add" class="button access-source-add" type="button" data-access-action="open-create">+ 新增接入源</button><button id="access-source-refresh" class="button-icon access-source-refresh" type="button" aria-label="刷新">↻</button></div></div>' +
  ...
  function actions(r, u) {
    if (rowType(r) === "gb_device") {
      return '<button class="table-action table-action-link access-source-action-link" type="button" data-access-action="refresh-row" data-access-id="' + u.escapeAttribute(r.id) + '">刷新</button><span class="access-source-action-divider">|</span><button class="table-action table-action-link access-source-action-link" type="button" data-route="gb-channel-list">通道</button><span class="access-source-action-divider">|</span><button class="table-action table-action-link access-source-action-link" type="button" data-route="gb-device-edit">编辑</button><span class="access-source-action-divider">|</span><button class="table-action table-action-link table-action-danger access-source-action-link" type="button" data-access-action="delete-row" data-access-id="' + u.escapeAttribute(r.id) + '">删除</button>';
    }

    return '<button class="table-action table-action-link access-source-action-link" type="button" data-access-action="play" data-access-id="' + u.escapeAttribute(r.id) + '">播放</button><span class="access-source-action-divider">|</span><button class="table-action table-action-link access-source-action-link" type="button" data-route="stream-proxy-edit" data-access-action="proxy-edit" data-access-id="' + u.escapeAttribute(r.id) + '">编辑</button><span class="access-source-action-divider">|</span><button class="table-action table-action-link access-source-action-link" type="button" data-route="cloud-record" data-access-action="open-cloud-record" data-access-id="' + u.escapeAttribute(r.id) + '">云端录像</button><span class="access-source-action-divider">|</span><button class="table-action table-action-link table-action-danger access-source-action-link" type="button" data-access-action="delete-row" data-access-id="' + u.escapeAttribute(r.id) + '">删除</button>';
  }
