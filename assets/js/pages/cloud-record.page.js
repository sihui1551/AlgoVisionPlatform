(function () {
  window.registerPrototypePage({
    key: "cloud-record",
    styleSource: "../assets/css/pages/cloud-record.css",
    kind: "dashboard",
    heading: "云端录像",
    subtitle: "录像播放器",
    ...
    productDoc: {
      title: "云端录像需求说明",
      summary: "面向研发、设计、测试说明录像回放页的原型范围。",
      goal: "供运营人员从接入源管理进入云端录像回放页，重点支持查看当前录像源、基础播放控制和回放操作占位。",
      modules: [
        "播放器区域用于展示当前选中的录像源名称和回放画面占位。",
        "信息栏展示码率信息及截图、刷新、全屏等常用工具入口。",
        "时间轴和控制栏用于表达快退、播放、快进和倍速等回放操作。"
      ],
      rules: [
        "当前页面以播放器演示为主，不承载真实视频流播放能力。",
        "录像源信息需要与接入源管理页当前选中的拉流设备保持同步。",
        "播放控制按钮用于表达回放能力范围，不在原型阶段处理真实媒体状态。"
      ],
      interactions: [
        "从接入源管理进入页面后，需要正确显示当前选中的录像源名称与流地址。",
        "点击截图、刷新、全屏和播放控制按钮时，需要给出对应反馈。",
        "未选择录像源时，页面需展示默认占位状态。"
      ]
    },
