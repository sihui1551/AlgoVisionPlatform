(function () {
  ...
  window.registerPrototypePage({
    key: "gb-channel-list",
    styleSource: "../assets/css/pages/gb-channel-list.css",
    kind: "table",
    heading: "通道列表",
    ...
    productDoc: {
      title: "通道列表需求说明",
      summary: "面向研发、设计、测试说明国标通道列表页的原型范围。",
      goal: "供平台管理员查看设备通道基础信息，重点支持搜索筛选、码流配置和通道预览入口。",
      modules: [
        "顶部工具栏提供返回、搜索、通道类型、在线状态和码流类型筛选入口。",
        "通道列表集中展示名称、编号、快照、厂家、位置、音频、码流类型和在线状态。",
        "行内操作承接详情、编辑、播放预览等能力，浮动菜单用于收纳扩展动作。",
        "播放器弹窗用于查看实时画面、云台、编码信息和语音对讲相关占位。"
      ],
      rules: [
        "列表页以通道浏览和轻量维护为主，不在当前页直接展开完整表单编辑。",
        "码流类型筛选与行内码流配置需要分别表达查询条件和当前配置结果。",
        "播放器弹窗展示能力范围即可，原型阶段不处理真实媒体播放。"
      ],
      interactions: [
        "搜索和筛选条件变化后，列表结果需要正确刷新。",
        "点击行内详情或编辑入口时，需要打开对应弹窗或页面。",
        "点击播放预览后，需要打开播放器弹窗并支持页签切换。",
        "行内码流类型选择变化后，需要保留当前行的配置结果。"
      ]
    },
    emptyText: "暂无通道数据",
    gbChannelPage: {
