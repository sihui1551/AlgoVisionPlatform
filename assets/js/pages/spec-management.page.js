(function () {
  ...
  window.registerPrototypePage({
    ...
    productDoc: {
      title: "规格管理需求说明",
      summary: "面向研发、设计、测试说明资源规格管理页的原型范围。",
      goal: "供平台管理员统一维护平台可选资源规格，重点支持规格检索、创建和状态管理。",
      modules: [
        "顶部工具栏提供新建规格、筛选和关键字搜索入口。",
        "规格列表集中展示规格 ID、名称、CPU、内存、GPU、显存、硬盘和状态信息。",
        "创建弹窗用于录入规格名称、简介以及 CPU、内存、GPU、显存和硬盘配置。",
        "详情弹窗用于核对规格基础信息与当前状态。"
      ],
      rules: [
        "列表页负责规格检索和状态切换，创建动作在弹窗中完成。",
        "CPU、内存、显存、GPU 数量和硬盘大小使用正整数校验。",
        "禁用规格后不删除记录，仅阻止后续新任务继续选择该规格。"
      ],
      interactions: [
        "点击新建规格可打开创建弹窗，保存后列表即时刷新。",
        "点击行内详情可打开规格详情弹窗。",
        "点击启用或禁用会切换当前规格状态。",
        "点击删除后会从当前原型列表中移除该规格。"
      ]
